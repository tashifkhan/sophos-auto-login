...
def logout(credential: Credential, timeout: int = 5) -> Union[bool, str]:
    ...
    try:
        with requests.Session() as s:
            p = s.post(
                "http://172.16.68.6:8090/httpclient.html", data=payload, timeout=timeout
            )
            if p.status_code == 200:
                ...
                if message_element is not None:
                    ...
                    if message_text == "You&#39;ve signed out":
                        send_notification(
                            "Sophos Auto Login",
                            f"{username} have been logged out",
                        )
                        return True
                else:
        ...
        send_notification(
            "Sophos Auto Login",
            f"Timeout occurred while trying to logout {username}",
        )
        return "Fail"
        ...
        send_notification(
            "Sophos Auto Login",
            f"Error logging out {username}: {e}",
        )
        return False
