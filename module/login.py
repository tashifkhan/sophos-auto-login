...
def login(credentials: list[Creditial]) -> tuple[bool, int]:
    ...
    for i, cred in enumerate(credentials):
        ...
        with requests.Session() as s:
            p = s.post(
                "http://172.16.68.6:8090/httpclient.html", data=payload, timeout=45
            )
            if p.status_code == 200:
                ...
                if message_element is not None:
                    ...
                    if (
                        ...
                        print(
                            f"Login failed for {username}. Trying the next credentials.\n"
                        )
                        send_notification(
                            "Sophos Auto Login", f"{username} login failed"
                        )
                        ...
                        if check_internet_connection():
                            try:
                                ...
                                download_mbps, upload_mbps, ping_ms, server_info = (
                                    speed_results
                                )
                                ...
                                send_notification(
                                    "Sophos Auto Login - Connected", speed_message
                                )

                            except Exception as e:
                                print(f"Speed test failed: {str(e)}")
                                send_notification(
                                    "Sophos Auto Login - Connected",
                                    f"Connected using {username}\nSpeed test failed: {str(e)}",
                                )
                        ...
                        print(
                            "Unknown response:", message_text, "\nusername:", username
                        )
                        send_notification(
                            "Sophos Auto Login", f"Unknown response for {username}"
                        )
                    ...
                    send_notification(
                        "Sophos Auto Login",
                        f"Error {username} - {"Message element not found."}",
                    )
