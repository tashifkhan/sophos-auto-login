...
def exit_handler(cred_index: int | None = None, credentials: Creditial = None, signal=None, frame=None):
    ...
    print("\nExiting...")
    try:
        if cred_index is not None and credentials and 0 <= cred_index < len(credentials):
            print(f"Logging out active credential: {credentials[cred_index]['username']}")
            
            result = logout(credentials[cred_index])
            if result == "Fail":
                print("Warning: Timeout occurred while logging out.")
                send_notification("Sophos Auto Login", "Timeout occurred during logout.")
                fail = True
            elif result is False:
                print("Warning: Failed to logout user.")
                send_notification("Sophos Auto Login", "Failed to logout user.")
                fail = True
            else:
                fail = False
                print(f"Successfully logged out: {credentials[cred_index]['username']}")
        ...
        if not fail:
            ...
            send_notification("Sophos Auto Login", "Logout failed. Exited.")
